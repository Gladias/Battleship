...
class GameObject:
    ...
    def update_screen(self, text_list, ships, buttons, first_board, second_board, screen):
        ...
        if first_board.is_visible():
            for row in first_board.board:
                for cell in row:
                    if cell.is_ship_on():
                        pygame.draw.rect(screen, const.SHIP_COLOR,
                                         cell.screen_position + (const.CELL_SIZE, const.CELL_SIZE))
                    if cell.sign == Sign.CLEAR and not cell.is_ship_on():
                        pygame.draw.rect(screen, const.CELL_COLOR,
                                         cell.screen_position + (const.CELL_SIZE, const.CELL_SIZE))
                    elif cell.sign == Sign.MISS:
                        pygame.draw.rect(screen, const.MISS_COLOR,
                                         cell.screen_position + (const.CELL_SIZE, const.CELL_SIZE))
                    elif cell.sign == Sign.HIT:
                        pygame.draw.rect(screen, const.HIT_COLOR,
                                         cell.screen_position + (const.CELL_SIZE, const.CELL_SIZE))
                    elif cell.sign == Sign.SUNK:
                        self.draw_sunken_ship(screen, cell)
    ...
    def draw_sunken_ship(self, screen, cell):
    ...
    def draw_finish_menu(self, screen, font, win: bool):
        if win:
            result_caption = Text("Wygrana!", font, const.RESULT_POSITION)
        else:
            result_caption = Text("Przegrana!", font, const.RESULT_POSITION)
        play_again_button = Button(pygame.Rect(const.PLAY_AGAIN_BUTTON), "Zagraj ponownie!", font,
                                   const.PLAY_AGAIN_COLOR)
...
class Text(GameObject):
    ...
    def update_info(self, info, font, screen):
