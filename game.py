                            ...
                            for row in self.first_board.board:
                                for cell in row:
                                    if cell.rect.collidepoint(mouse_pos):
                                        ...
                                        if not self.first_board.check_if_edge_of_board(cell, self.active_ship, True) and not self.first_board.check_if_another_ship_is_placed(cell, self.active_ship, True):
                                            
                                            for i in range(self.active_ship):
                                                self.first_board.board[cell.coordinates[0]][i + cell.coordinates[1]].put_ship()
                                            self.active_ship = 0

                interface.update_screen(text_list, ships, self.first_board, self.second_board, self.screen)
